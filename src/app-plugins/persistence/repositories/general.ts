...
export default abstract class GeneralDBCommands<T extends Document, K> {
  // db instance
   ...
   * @param id 
   */
  public findOne (query: K, projection?: Partial<Record<keyof K, 0|1>>, toObject: boolean = true): Promise<T> {
    return this.collectionModel.findOne(query, projection)
      .then((data) => {
        if (!data) {
          throw new Error('No data found.')
        }
        return toObject ? <any>data.toObject() : <any>data
      })
  }
  /**
   * insert data 
  ...
  public async updateOne(query: Record<keyof K, any>, data: K) {
    try {
      ...
      document.set(data)
      await document.save()
      return <T> document.toObject() as any
    } catch (error) {
      throw error
    }
  }
